 ...
 * - pass: 모든 레이어 통과
 * - constraint: format, semantic 통과, constraint 실패
 * - semantic: format 통과, semantic 실패
 * - format: format 실패
 */
export const getLayerStatus = (
  result: RunResultRow,
  layer: "semantic" | "constraint",
): LayerStatus => {
  if (!result.isFormatPassed) return "skipped";

  if (layer === "semantic") {
    // semantic 통과: status가 pass 또는 constraint (semantic까지는 통과)
    return result.status === "pass" || result.status === "constraint" ? "pass" : "fail";
  }

  // constraint 통과: status가 pass
...
export const isSemanticPassed = (result: RunResultRow): boolean => {
  return result.status === "pass" || result.status === "constraint";
};
